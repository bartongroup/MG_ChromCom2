...
#'
#' @return Tibble with dot data
process_dots_intensities <- function(r, stats, intensity_prefix, track_colour) {
  map_dfr(stats, function(stat) {
    ch1 <- r[[str_glue("Intensity {stat} Ch=1 Img=1")]] |>
      select(
        red = str_glue("Intensity {stat}"),
        track_id = TrackID,
        id = ID
      ) |>
      mutate(track_id = as.character(as.integer(track_id)))

    ch2 <- r[[str_glue("Intensity {stat} Ch=2 Img=1")]] |>
      select(
        green = str_glue("Intensity {stat}"),
        id = ID
      )
...
#' @param track_colour Table with colour per track
#'
#' @return Tibble with colour data
...
#' @param what Which field to extract
#'
#' @return Tibble
#' @export
merge_cell_data <- function(d, what = "dat") {
  map(names(d), function(cellid) {
    d[[cellid]][[what]] |>
...
#' object (created by read_cells) and creates a list of coordinates and
#' metadata.
#'
#' @param rw Object with raw data.
#' @param z_correction Correction to z coordinates due to different refraction
#'   in oil-based objective and water-based medium with cells.
#' @param dots_stats Vector of statistics for dots intensities, e.g. "Mean" or
#'   "Median"
#' @param extvol_stats Vector of statistics for extvol intensities, e.g. "Mean"
#'   or "Median"
#'
#' @return List of metadata, xyz, parsed, params and (optional) celldat. Only
#'   xyz contains corrected z-coordinate, celldat contains original raw
#'   coordinates.
#' @export
process_raw_data <- function(rw, z_correction = 0.85,
                             dots_stats = c("Sum", "Max", "Mean"),
                             extvol_stats = c("Sum", "Mean", "Min")) {
  md <- rw$metadata |> select(cell_id, cell_line, condition, movie, cell, cellcon, mcell, rupture, rupture_frame)
